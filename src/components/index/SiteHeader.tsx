...
export default function SiteHeader({ scrolled, menuOpen, setMenuOpen, openBooking }: SiteHeaderProps) {
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "glass shadow-sm py-3" : "py-5"}`}>
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => openBooking("Шапка сайта — Записаться")} className="hidden md:inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-semibold text-white gradient-pink hover:opacity-90 transition-opacity shadow-md">
            Записаться
          </button>
      ...
      {menuOpen && (
        <div className="lg:hidden glass border-t border-denu-pink/10 mt-2 px-4 pb-6 pt-4">
          ...
          <button onClick={() => { setMenuOpen(false); openBooking("Мобильное меню — Записаться онлайн"); }} className="mt-5 flex items-center justify-center gap-2 px-5 py-3 rounded-full font-semibold text-white gradient-pink w-full">
            Записаться онлайн
          </button>
        </div>
      )}
